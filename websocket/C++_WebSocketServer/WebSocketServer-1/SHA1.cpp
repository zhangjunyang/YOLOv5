...
#ifdef SHA1_UTILITY_FUNCTIONS
bool CSHA1::ReportHash(TCHAR* tszReport, UINT_8* szOut, REPORT_TYPE rtReportType) const
{
	...
	else if(rtReportType == REPORT_UINT8)
	{
		for (int i = 0; i < 20; ++i)
		{
			szOut[i] = m_digest[i];
...
bool CSHA1::ReportHashStl(std::basic_string<TCHAR>& strOut, UINT_8* szOut, REPORT_TYPE rtReportType) const
{
	TCHAR tszOut[84];
	const bool bResult = ReportHash(tszOut, szOut, rtReportType);
